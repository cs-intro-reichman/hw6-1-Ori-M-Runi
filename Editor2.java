...
public class Editor2 {
	...
	public static void main (String[] args){
		...
		Color[][] scaledImg = Runigram.scaled(imageIn, width, height);	
		
		// Creates a canvas in which the first image will be displayed and displays that img  
		Runigram.setCanvas(imageIn);
		Runigram.display(imageIn);
		// pauses for a few seconds.
		StdDraw.pause(3000);
		// Creates a canvas in which the scaled image will be displayed and displays that img
		Runigram.setCanvas(scaledImg);
		Runigram.display(scaledImg);							
	}
}
